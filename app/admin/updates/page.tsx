...
export default function AdminUpdatesPage() {
  ...
  return (
    <div className="space-y-5">
      <form
        onSubmit={post}
        className="rounded-xl border border-turf-line bg-turf-panel p-4"
      >
        ...
        <textarea
          ...
          className="w-full rounded-md border border-turf-line bg-turf-deep px-3 py-2 text-sm outline-none focus:border-accent"
        />

        {/* Optional image (sponsor banner / visual update) */}
        <div className="mt-2">
          <label className="block font-mono text-[10px] uppercase tracking-widest text-chalk-mut">
          ...
          <div className="mt-1 flex items-center gap-3">
            <input
              ...
              className="block w-full text-sm text-chalk file:mr-3 file:rounded-md file:border-0 file:bg-turf-deep file:px-3 file:py-2 file:font-mono file:text-xs file:uppercase file:text-accent"
            />
            {image && (
              <button
                type="button"
                onClick={clearImage}
                className="shrink-0 rounded border border-turf-line px-2 py-1 font-mono text-[10px] uppercase text-chalk hover:border-red-500/50 hover:text-red-300"
              >
                Remove
              </button>
            )}
          </div>
          {imgBusy && <p className="mt-1 text-xs text-chalk-mut">Compressing…</p>}
          {image && (
            <div className="mt-2">
              ...
              <img
                src={image.dataUrl}
                alt="preview"
                className="max-h-48 w-full rounded-lg border border-turf-line bg-turf-deep object-contain"
              />
        ...
        <button
          type="submit"
          disabled={!canPost}
          className="mt-2 rounded-md bg-accent px-4 py-2 font-display text-sm font-bold uppercase tracking-wide text-turf-deep disabled:opacity-50"
        >
          {busy ? "Posting…" : "Post"}
        </button>
      </form>
      ...
      <div className="space-y-2">
        {updates.map((u) => (
          <div
            key={u.id}
            className="flex items-start justify-between gap-3 rounded-lg border border-turf-line bg-turf-panel p-3"
          >
            <div className="min-w-0 flex-1">
              ...
              {u.image_url && (
                ...
                <img
                  ...
                  loading="lazy"
                  className="mt-2 max-h-64 w-full rounded-lg border border-turf-line bg-turf-deep object-contain"
                />
              )}
              {u.body && (
        ...
        {updates.length === 0 && (
          <p className="text-sm text-chalk-mut">No announcements yet.</p>
        )}
      </div>
    </div>
  );
